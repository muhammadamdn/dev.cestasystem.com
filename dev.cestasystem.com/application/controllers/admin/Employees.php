...
class Employees extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if ($comp_id == 1) {
				...
				$data['all_user_roles'] = $this->Roles_model->my_company_roles($comp_id);
		}
	...
	public function hr() {
      ...
      //  $config['next_link'] = 'Next';
        ...
        $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
				...
				if ($comp_id == 1) {
					$data["results"] = $this->Employees_model->fetch_all_employees($config["per_page"], $page);
				} else {
					$data["results"] = $this->Employees_model->fetch_all_my_employees($config["per_page"], $page, $comp_id);
				}
				// End Cesta
    ...
    public function employees_list()
     {
		...
		if ($comp_id == 1) {
				...
				$employee = $this->Employees_model->get_my_employees($comp_id);
		}
		// End Cesta
        ...
        foreach($employee->result() as $r) {
			...
			if($r->user_id != '1') {
				...
				$function = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a href="'.site_url().'admin/employees/detail/'.$r->user_id.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"><i class="fa fa-arrow-circle-right"></i></button></a></span>'.$option.'';
	...
	public function add_employee() {
		...
		} else if(empty($this->input->post('date_of_birth'))) {
	...
	public function social_info() {
		...
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		if (!empty($this->input->post('facebook_link')) && !filter_var($this->input->post('facebook_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_fb_field_error');
		} else if (!empty($this->input->post('twitter_link')) && !filter_var($this->input->post('twitter_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_twitter_field_error');
		} else if (!empty($this->input->post('blogger_link')) && !filter_var($this->input->post('blogger_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_blogger_field_error');
		} else if (!empty($this->input->post('linkdedin_link')) && !filter_var($this->input->post('linkdedin_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_linkedin_field_error');
		} else if (!empty($this->input->post('google_plus_link')) && !filter_var($this->input->post('google_plus_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_gplus_field_error');
		} else if (!empty($this->input->post('instagram_link')) && !filter_var($this->input->post('instagram_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_instagram_field_error');
		} else if (!empty($this->input->post('pinterest_link')) && !filter_var($this->input->post('pinterest_link'), FILTER_VALIDATE_URL)) {
			$Return['error'] = $this->lang->line('xin_hr_pinterest_field_error');
		} else if (!empty($this->input->post('youtube_link')) && !filter_var($this->input->post('youtube_link'), FILTER_VALIDATE_URL)) {
	...
	public function contact_info() {
		...
		if($this->input->post('relation')==='') {
			...
			$Return['error'] = $this->lang->line('xin_hr_string_error');
		} else if(!empty($this->input->post('contact_no')) && !preg_match('/^([0-9]*)$/', $this->input->post('contact_no'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		} else if(!empty($this->input->post('work_phone')) && !preg_match('/^([0-9]*)$/', $this->input->post('work_phone'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		} else if(!empty($this->input->post('work_phone_extension')) && !preg_match('/^([0-9]*)$/', $this->input->post('work_phone_extension'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		} else if($this->input->post('mobile_phone')==='') {
			 $Return['error'] = $this->lang->line('xin_employee_error_mobile');
		} else if(!preg_match('/^([0-9]*)$/', $this->input->post('mobile_phone'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		} else if(!empty($this->input->post('home_phone')) && !preg_match('/^([0-9]*)$/', $this->input->post('home_phone'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		} else if(empty($this->input->post('work_email'))) {
			 $Return['error'] = $this->lang->line('xin_employee_error_email');
		} else if (!filter_var($this->input->post('work_email'), FILTER_VALIDATE_EMAIL)) {
			$Return['error'] = $this->lang->line('xin_employee_error_invalid_email');
		} else if (!empty($this->input->post('personal_email')) && !filter_var($this->input->post('personal_email'), FILTER_VALIDATE_EMAIL)) {
			$Return['error'] = $this->lang->line('xin_employee_error_invalid_email');
		} else if(!empty($this->input->post('zipcode')) && !preg_match('/^([0-9]*)$/', $this->input->post('zipcode'))) {
			 $Return['error'] = $this->lang->line('xin_hr_numeric_error');
		}
