...
class Roles extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('999',$role_resources_ids)) {
			if(!empty($session)){
    ...
    public function role_list()
     {
		...
		if ($comp_id == 1) {
		  ...
		  $role = $this->Roles_model->get_comp_roles($comp_id);
		}
		// End Cesta
		...
		$data = array();
          ...
          foreach($role->result() as $r) {
			...
			//edit
			if($r->role_id==1){
				$roleAccess = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-role_id="'. $r->role_id . '"><i class="fa fa-pencil-square-o"></i></button></span>';
			} else {
				$roleAccess = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-role_id="'. $r->role_id . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->role_id . '"><i class="fa fa-trash-o"></i></button></span>';
			}
