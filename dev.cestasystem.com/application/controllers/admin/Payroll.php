...
class Payroll extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function templates()
     {
		...
		if ($comp_id == 1) {
		    ...
		    $data['all_companies'] = $this->Xin_model->get_my_companies($comp_id);
		}
		// End Cesta
    ...
    public function advance_salary_report_list()
		...
		$data = array();
          ...
          foreach($payslip->result() as $r) {
			...
			if($payment_check->num_rows() > 0){
				...
				$p_details = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".detail_modal_data" data-employee_id="'. $r->user_id . '" data-pay_id="'. $make_payment[0]->make_payment_id . '" data-company_id="'.$this->input->get("company_id").'"><i class="fa fa-arrow-circle-right"></i></button></span>';
				} else {
					if($net_salary > 0) {
					$functions = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_payroll_make_payment').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".'.$p_class.'" data-employee_id="'. $r->user_id . '" data-payment_date="'. $p_date . '" data-company_id="'.$this->input->get("company_id").'"><i class="fa fa-money"></i></button></span>';
					} else {
	 ...
	 public function salary_list()
     {
		...
		$hourly_rate = '';
		$hr_radio = '
		<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_payroll_select_hourly').'"><label class="display-inline-block custom-control custom-checkbox">
