		...
		$data = array();
          ...
          foreach($language->result() as $r) {
			...
			if($wz_lang == 'english' || $r->language_id=='1' ){
				...
				if($r->is_active==1){
					$success = '<button type="button" data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_employees_inactive').'" class="btn btn-icon btn-pure success btn-act-lang active-lang mr-1" data-language_id="'. $r->language_id . '" data-is_active="0"><i class="fa fa-check-circle"></i></button>';
				} else {
					$success = '<button type="button" data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_employees_active').'" class="btn btn-icon btn-pure danger btn-act-lang active-lang mr-1" data-language_id="'. $r->language_id . '" data-is_active="1"><i class="fa fa-times-circle-o"></i></button>';
				}
			$del = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->language_id . '"><i class="fa fa-trash-o"></i></button></span>';
			}  			
			$data[] = array(
				$del,
				$name_flg,
