...
class awards_model extends CI_Model {
  ...
  // Start Cesta
  // get my employes
  public function get_my_awards($comp_id)
	{
    $comp_id = array('company_id' => $comp_id);
	  return $this->db->get_where('xin_awards', $comp_id);
	}
  // End Cesta
	 public function read_award_type_information($id) {

		$condition = "award_type_id =". $this->db->escape($id);
		$this->db->select('*');
	...
	public function read_award_information($id) {
		...
		$condition = "award_id =". $this->db->escape($id);
		$this->db->select('*');
