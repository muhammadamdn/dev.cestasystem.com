<?php
	...
	public function get_comp_template($cid,$id) {
		return $query = $this->db->query("SELECT * from xin_employees where company_id='".$cid."'");
	}
	// get payroll templates > employee/company
	public function get_employee_comp_template($cid,$id) {
		return $query = $this->db->query("SELECT * from xin_employees where company_id='".$cid."' and user_id='".$id."'");
	}
	...
	public function advance_salaries_report_view($id) {
	  $query = $this->db->query("SELECT advance_salary_id,company_id,employee_id,month_year,one_time_deduct,monthly_installment,reason,status,total_paid,is_deducted_from_salary,created_at,SUM(`xin_advance_salaries`.advance_amount) AS advance_amount FROM `xin_advance_salaries` where status=1 and employee_id='".$id."' group by employee_id");
	  return $query->result();
	}
