...
class Xin_model extends CI_Model {
	...
	public function select_module_class($mClass,$mMethod) {
		...
		// admin menu
			...
			$arr['adm_open'] = 'open';
			return $arr;
		} else if($mMethod=='office_shift') {
			$arr['offsh_active'] = 'active';
			$arr['adm_open'] = 'open';
			return $arr;
		} else if($mMethod=='holidays') {
	...
	public function read_user_attendance_info() {
		return $query = $this->db->query("SELECT * from xin_employees where user_id = '0000'");
	}
	...
	public function read_theme_info($id) {
		...
		$condition = "theme_settings_id =" . "'" . $id . "'";
		$this->db->select('*');
		$this->db->from('xin_theme_settings');
		$this->db->where($condition);
		$this->db->limit(1);
		$query = $this->db->get();
	...
	// Start Cesta
	// get my companies
	public function get_my_companies($comp_id)
	{
		$query = $this->db->query("SELECT * from xin_companies WHERE company_id=$comp_id");
			return $query->result();
	}
	// End Cesta
	...
	public function read_dep_designations($id) {
		...
		$query = $this->db->query("SELECT * from xin_designations where department_id = '".$id."'");
  	 	return $query->result();
	}
	...
	public function read_designation_employees($id) {
		...
		$query = $this->db->query("SELECT * from xin_employees where designation_id = '".$id."'");
  	 	return $query->result();
	}
	...
	public function current_month_day_attendance($current_month) {
		...
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT * from xin_attendance_time where attendance_date = '".$current_month."'");
		return $query->num_rows();
	}
	...
	public function current_employee_absent_calendar($current_month) {
		...
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT at.*,e.*,la.* from xin_attendance_time as at, xin_employees as e, xin_leave_applications as la where at.attendance_date = '".$current_month."' and e.user_id!=at.employee_id and e.user_id!=la.employee_id");
		return $query->result();
	}
	...
	public function employee_leave_date_calendar($current_date) {
		...
		$query = $this->db->query("SELECT la.*,e.user_id,e.first_name,e.last_name from xin_leave_applications as la, xin_employees as e where (la.from_date between la.from_date and la.to_date) or la.from_date = '".$current_date."' and la.to_date = '".$current_date."' and e.user_id=la.employee_id and la.status=2 group by la.employee_id");
		return $query->result();
	}
	...
	public function current_employee_working_calendar($current_date) {
		...
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT * from xin_attendance_time where attendance_date = '".$current_date."' group by employee_id");
		return $query->result();
	}
	...
	{
	  $query = $this->db->query("SELECT * from xin_employees where gender='Male'");
	...
	{
	  $query = $this->db->query("SELECT * from xin_employees where gender='Female'");
	...
	{
	  $query = $this->db->query("SELECT * from xin_languages where is_active = 1 order by language_name asc");
  	  return $query->result();
	}
	...
	public function get_single_unread_message($to_id) {
		$query = $this->db->query("SELECT * FROM xin_chat_messages where to_id='".$to_id."' and is_read=0");
		return $query->num_rows();
	}
