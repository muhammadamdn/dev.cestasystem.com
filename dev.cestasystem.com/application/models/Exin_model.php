...
class Exin_model extends CI_Model {
	...
	public function total_employee_expense_dash() {
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT * from xin_expenses where employee_id = '".$session['user_id']."'");
		return $query->num_rows();
	}
	...
	public function total_employee_travel_dash() {
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT * from xin_employee_travels where employee_id = '".$session['user_id']."'");
		return $query->num_rows();
	}
	
	// task completed
	public function get_completed_tasks($task_id) {
		$query = $this->db->query("SELECT * from xin_tasks where task_id = '".$task_id."' and task_status='2'");
		return $query->num_rows();
	}
	// task overdue
	public function get_overdue_tasks($task_id) {
		$date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_tasks where task_id = '".$task_id."' and end_date < '".$date."' and task_status!='2'");
		return $query->num_rows();
	}
	// task todo
